...
import { sampleWithRequiredData } from '../task-node.test-samples';
import { TaskNodeService } from '../service/task-node.service';

import { TaskNodeComponent } from './task-node.component';
import SpyInstance = jest.SpyInstance;

describe('TaskNode Management Component', () => {
  let comp: TaskNodeComponent;
  ...
  beforeEach(() => {
    TestBed.configureTestingModule({
      ...
      .overrideTemplate(TaskNodeComponent, '')
      .compileComponents();
  ...
  it('Should call load all on init', () => {
    ...
    expect(comp.taskNodes?.[0]).toEqual(expect.objectContaining({ id: 'ABC' }));
  });

  describe('trackId', () => {
    it('Should forward to taskNodeService', () => {
      const entity = { id: 'ABC' };
      jest.spyOn(service, 'getTaskNodeIdentifier');
      const id = comp.trackId(entity);
      expect(service.getTaskNodeIdentifier).toHaveBeenCalledWith(entity);
